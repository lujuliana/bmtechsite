...
const openJobSlugs = new Set(jobs.filter((job) => job.isOpen).map((job) => job.slug));
...
export function resolveLocaleRoute(path: string, locale: Locale): string | undefined {
  const normalizedPath = normalizePath(path);
  const staticRoute = localizedStaticRoutes.find((route) =>
    Object.values(route).some((routePath) => normalizePath(routePath) === normalizedPath),
  );
  ...
  if (locale === 'en' && jobMatch && (!jobMatch[1] || openJobSlugs.has(jobMatch[1]))) {
...
export function isActivePath(
  ...
  return current === target || (match === 'descendant' && current.startsWith(`${target}/`));
}
